...
function LeadSight.drawFlatCross(color, position, normalVector, distance, size, width)
	...
	local y1 = LeadSight.map2dVectorTo3d(i, j, position, vec2(0, - s))
...
function LeadSight.refreshWeaponsData(craft)
	local minProjectileSpeed = 1000000
...
function LeadSight.sineCosine(sineCosineValue)
	local clamped = math.max(-1, math.min(1, sineCosineValue))
...
function LeadSight.getPlaneBasisVectors(plane)
	...
	if math.abs(C) > eps then
		genericPoint = vec3(0.0, 0.0, -D / C)
	else
		genericPoint = vec3(0.0, 0.0, 0.0)
	end

	if math.abs(B) > eps then
		genericPoint2 = vec3(0.0, -D / B, 0.0)
	else
		genericPoint2 = vec3(1.0, 0.0, 0.0)
	end
	
	local i = genericPoint - genericPoint2
	local iLen = LeadSight.length(i)
	if iLen < eps then
		i = vec3(1, 0, 0)
		iLen = 1
	end
...
function LeadSight.projectVectorOnPlane(vector, plane)
	local denom = plane.x * plane.x + plane.y * plane.y + plane.z * plane.z
	if denom < eps then
		return vector
	end

	local t = - (plane.x * vector.x + plane.y * vector.y + plane.z * vector.z + plane.w) / denom
